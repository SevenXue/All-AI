...
class BuildingClassify:
    def __init__(self, dim, epochs, batch_size, model_file):
        ...
        plot_model(self.classify, to_file='view/classify_16_double.png', show_shapes=True)
...
def visual_data_as_img(data_file, labels, save=True):
    with open(data_file, 'r') as df:
        os.makedirs('datasets/classify_double', exist_ok=True)
        lines = df.readlines()
        index = 0
        for line in lines:
            ...
            for i in range(len(buildings)):
                ...
                index += 1
            if save:
                plt.savefig(f'datasets/classify_double/{data_id}.jpg')
            else:
                plt.show()
            plt.close()

def get_classify_model(data_file, npz_file, is_testing=False):
    model_file = 'model/classify_mp_maxmin_double.h5'
